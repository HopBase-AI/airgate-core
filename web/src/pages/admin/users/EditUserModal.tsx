...
export function EditUserModal({ open, user, onClose, onSubmit, loading }: EditUserModalProps) {
  ...
  const [form, setForm] = useState<UpdateUserReq>({
    ...
    is_enterprise_owner: user.is_enterprise_owner ?? false,
          ...
          <Modal.Dialog className="ag-elevation-modal">
            ...
            <Modal.Body>
              <div className="space-y-4">
                ...
                {editableRole !== 'admin' && (
                  <NativeSwitch
                    isSelected={!!form.is_enterprise_owner}
                    contentClassName="text-xs"
                    label={t('users.is_enterprise_owner')}
                    onChange={(v) => setForm({ ...form, is_enterprise_owner: v })}
                  />
                )}
              </div>
            </Modal.Body>
