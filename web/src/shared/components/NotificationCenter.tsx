    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <section
          ...
          aria-label={selectedNotification ? t('notifications.detail_title') : t('notifications.inbox_title')}
            ...
            <>
              <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-2">
                <Tooltip>
                  <Tooltip.Trigger>
                    <Button
                      aria-label={t('notifications.back_to_list')}
                      className="shrink-0"
                      isIconOnly
                      size="sm"
                      variant="ghost"
                  ...
                  <Tooltip.Content>{t('notifications.back_to_list')}</Tooltip.Content>
                </Tooltip>
              ...
              <div className="min-h-0 overflow-y-auto px-4 py-4">
                <div className="flex items-start gap-3">
                  {SelectedLevelIcon && (
                    <span
                      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius)] bg-bg"
                      style={{ color: LEVEL_COLORS[selectedNotification.level] }}
                    >
                      <SelectedLevelIcon className="h-4 w-4" />
                    </span>
                  )}
                  <div className="min-w-0 flex-1">
                    <h3 className="break-words text-sm font-semibold leading-5 text-text">
                      {selectedNotification.title || t('notifications.default_title')}
                    </h3>
