...
func (mq *MemberQuery) QueryAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MemberQuery) WithAPIKeys(opts ...func(*APIKeyQuery)) *MemberQuery {
	query := (&APIKeyClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := mq.withAPIKeys; query != nil {
...
func (mq *MemberQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Member, init func(*Member), assign func(*Member, *APIKey)) error {
	...
	query.Where(predicate.APIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.APIKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.member_api_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_api_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
