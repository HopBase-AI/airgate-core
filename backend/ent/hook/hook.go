...
// The SubscriptionReservationFunc type is an adapter to allow the use of ordinary
// function as SubscriptionReservation mutator.
type SubscriptionReservationFunc func(context.Context, *ent.SubscriptionReservationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SubscriptionReservationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SubscriptionReservationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SubscriptionReservationMutation", m)
}
