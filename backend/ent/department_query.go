...
func (dq *DepartmentQuery) QueryOwner() *UserQuery {
...
func (dq *DepartmentQuery) QueryMembers() *MemberQuery {
...
func (dq *DepartmentQuery) QueryAPIKeys() *APIKeyQuery {
...
func (dq *DepartmentQuery) WithOwner(opts ...func(*UserQuery)) *DepartmentQuery {
	query := (&UserClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) WithMembers(opts ...func(*MemberQuery)) *DepartmentQuery {
	query := (&MemberClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) WithAPIKeys(opts ...func(*APIKeyQuery)) *DepartmentQuery {
	query := (&APIKeyClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withOwner; query != nil {
		if err := dq.loadOwner(ctx, query, nodes, nil,
			func(n *Department, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withMembers; query != nil {
		if err := dq.loadMembers(ctx, query, nodes,
			func(n *Department) { n.Edges.Members = []*Member{} },
			func(n *Department, e *Member) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
...
func (dq *DepartmentQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Department, init func(*Department), assign func(*Department, *User)) error {
...
func (dq *DepartmentQuery) loadMembers(ctx context.Context, query *MemberQuery, nodes []*Department, init func(*Department), assign func(*Department, *Member)) error {
	...
	for _, n := range neighbors {
		fk := n.department_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DepartmentQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Department, init func(*Department), assign func(*Department, *APIKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.APIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.APIKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.department_api_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_api_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
