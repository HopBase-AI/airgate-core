...
type UserIdentity struct {
	...
	// 第三方平台标识：google / github
	Provider string `json:"provider,omitempty"`
	// 第三方平台的用户唯一 ID（Google sub / GitHub id）
	ProviderUserID string `json:"provider_user_id,omitempty"`
	// 绑定时第三方返回的邮箱，仅作展示与排查，不参与登录匹配
	...
	for i := range columns {
		...
		case useridentity.FieldProviderUserID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				ui.ProviderUserID = value.String
			}
		...
		case useridentity.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				ui.user_identities = new(int)
				*ui.user_identities = int(value.Int64)
			}
		default:
...
func (ui *UserIdentity) String() string {
	...
	builder.WriteString(ui.ProviderUserID)
	builder.WriteString(", ")
