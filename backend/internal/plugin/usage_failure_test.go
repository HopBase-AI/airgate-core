...
func TestFailureFromOutcome(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// recordedFailureStatus 与 failureFromOutcome 必须给出同一个状态码。
			if s := recordedFailureStatus(tt.execution); s != tt.wantStatus {
				t.Fatalf("recordedFailureStatus = %d, want %d", s, tt.wantStatus)
			}
		})
	}
}
...
func TestRedactCredentials(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "Basic 认证被抹掉但保留方案",
			input: "Proxy-Authorization: Basic dXNlcjpwYXNzd29yZA==",
			want:  "Proxy-Authorization: Basic [REDACTED]",
		},
		{
			name:  "api_key JSON 字段被抹掉",
			input: `{"api_key":"super-secret-value","ok":true}`,
			want:  `{"api_key":[REDACTED],"ok":true}`,
		},
		{
			name:  "token 查询参数被抹掉",
			input: "upstream?access_token=very-secret-token&retry=1",
			want:  "upstream?access_token=[REDACTED]&retry=1",
		},
		{
			name:  "Cookie header 被整体抹掉",
			input: "Cookie: session=abc123; csrf=def456; path=/",
			want:  "Cookie: [REDACTED]",
		},
		{
			name:  "未带字段名的 Gemini 39 位 key 被抹掉",
			input: "Gemini rejected AIzaabcdefghijklmnopqrstuvwxyz123456789",
			want:  "Gemini rejected [REDACTED]",
		},
		{
			name:  "40+ 位长随机串被抹掉",
