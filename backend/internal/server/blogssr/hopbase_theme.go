...
<span class="hb-featured-meta">{{if .PublishedAt}}<time>{{.PublishedAt}}</time>{{end}}{{if .ReadingTime}}<span>{{.ReadingTime}}</span>{{end}}</span>
...
{{if .CoverImage}}<img class="hb-featured-cover" src="{{.CoverImage}}" alt="" loading="eager">{{else}}<span class="hb-terminal-cover"><img src="{{$.LogoSrc}}" alt=""><span>hopbase / article.log</span><i aria-hidden="true"></i></span>{{end}}
</a>
{{end}}{{end}}
{{if .Rest}}<div class="hb-rows">{{range .Rest}}
<a class="hb-row" href="{{.URL}}">
<span class="hb-row-date">{{.PublishedAt}}</span>
...
:root{color-scheme:light;--bg:#f2f2f0;--fg:#0b0d0c;--muted:#62685f;--border:#c8c8c2;--card:#fbfbfa;--accent:#c54e08;--accent-fg:#fff;--accent-soft:#f8e7dc;--code-bg:#14150f;--code-fg:#d7dbd3;--maxw:760px;--mono:"IBM Plex Mono","SFMono-Regular",Menlo,Consolas,monospace;--shadow:0 14px 32px rgba(11,13,12,.11);--hb-canvas:#f2f2f0;--hb-paper:#fbfbfa;--hb-paper-2:#f4f4f2;--hb-hair:#e0e0dc;--hb-rule:#c8c8c2;--hb-ink:#0b0d0c;--hb-body:#4a4f49;--hb-muted:#62685f;--hb-terminal:#14150f;--hb-terminal-text:#d7dbd3;--hb-terminal-muted:#92998f;--hb-terminal-accent:#f28c3a;--hb-navw:1400px;--hb-ledgerw:1240px;--hb-ease:cubic-bezier(.16,1,.3,1)}
`

const hopBaseCSS = `
*{letter-spacing:0}
html{scroll-behavior:smooth;scroll-padding-top:70px;background:var(--hb-canvas)}
body{min-width:320px;background:var(--hb-canvas);color:var(--hb-ink);font-family:"IBM Plex Sans","PingFang SC","Noto Sans CJK SC","Microsoft YaHei",sans-serif;line-height:1.65}
...
.hb-header{position:sticky;z-index:30;top:0;height:70px;border-bottom:1px solid var(--hb-hair);background:rgba(242,242,240,.96);-webkit-backdrop-filter:saturate(160%) blur(14px);backdrop-filter:saturate(160%) blur(14px)}
...
.hb-brand b{font-size:16px;font-weight:600;text-transform:uppercase}
.hb-nav{display:flex;align-items:center;justify-content:center;gap:30px;margin-left:auto}
.hb-nav a{min-height:44px;display:inline-flex;align-items:center;border-bottom:2px solid transparent;color:var(--hb-body);font-family:var(--mono);font-size:12px;font-weight:500;text-transform:uppercase;white-space:nowrap}
.hb-nav a:hover,.hb-nav a.act{color:var(--hb-ink);border-bottom-color:var(--accent)}
.hb-actions{display:flex;align-items:center;gap:10px}
.hb-control,.hb-login,.hb-user,.hb-menu-button{box-sizing:border-box;min-height:40px;border:1px solid var(--hb-rule);border-radius:4px;background:transparent}
...
.hb-language-menu i{color:var(--accent);font-style:normal;opacity:0}
...
.hb-auth,.hb-auth-guest{display:inline-flex;align-items:center}
...
.hb-user-avatar{width:32px;height:32px;display:grid;place-items:center;border-radius:3px;background:var(--accent);color:#fff;font:500 12px/1 var(--mono)}
.hb-user-name{overflow:hidden;font-size:13px;font-weight:600;text-overflow:ellipsis;white-space:nowrap}
.hb-menu-button{width:44px;height:44px;padding:0;display:none;align-items:center;justify-content:center;color:var(--hb-ink);cursor:pointer}
...
.hb-mobile-menu[hidden]{display:none}.hb-mobile-menu nav{display:grid}.hb-mobile-menu a{min-height:48px;display:flex;align-items:center;border-bottom:1px solid var(--hb-hair);color:var(--hb-body);font-family:var(--mono);font-size:13px}.hb-mobile-menu a.act{color:var(--accent)}
...
.hb-intro .hb-frame{min-height:300px;padding-top:72px;padding-bottom:72px;display:grid;grid-template-columns:160px minmax(0,1fr);align-items:end}
...
.hb-intro h1{margin:0 0 18px;font-size:52px;font-weight:600;line-height:1.05;text-wrap:balance}
.hb-intro-copy>p{max-width:60ch;margin:0;color:var(--hb-body);font-size:17px;line-height:1.75}
.hb-ledger{padding-bottom:80px}
.hb-featured{min-height:360px;display:grid;grid-template-columns:144px minmax(0,1fr) minmax(300px,38%);border-bottom:1px solid var(--hb-rule);color:inherit;text-decoration:none}
.hb-featured-meta{padding:34px 24px 34px 0;display:flex;flex-direction:column;gap:10px;border-right:1px solid var(--hb-hair);color:var(--hb-muted);font-family:var(--mono);font-size:11px;font-variant-numeric:tabular-nums}
.hb-featured-copy{position:relative;min-width:0;padding:48px 48px;display:flex;flex-direction:column;align-items:flex-start;justify-content:center;border-right:1px solid var(--hb-hair)}
.hb-tag{margin-bottom:16px;color:var(--accent);font-family:var(--mono);font-size:11px;font-weight:500}
.hb-featured h2{max-width:760px;margin:0 0 18px;color:var(--hb-ink);font-size:36px;font-weight:600;line-height:1.18;text-wrap:balance;transition:color 120ms var(--hb-ease)}
.hb-featured-copy>p{max-width:64ch;margin:0;color:var(--hb-body);font-size:15px;line-height:1.72;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;overflow:hidden}
...
.hb-featured:hover h2{color:var(--accent)}.hb-featured:hover .hb-read-arrow{color:var(--accent);transform:translate(4px,-4px)}
.hb-featured-cover{width:100%;height:100%;min-height:360px;display:block;object-fit:cover;background:var(--hb-paper-2)}
.hb-terminal-cover{position:relative;min-height:360px;padding:28px;display:flex;flex-direction:column;align-items:flex-start;justify-content:space-between;overflow:hidden;background:var(--hb-terminal);color:var(--hb-terminal-muted);font:500 11px/1.4 var(--mono)}
...
.hb-row{min-height:132px;display:grid;grid-template-columns:128px minmax(0,1fr) 190px;align-items:center;border-bottom:1px solid var(--hb-hair);color:inherit;text-decoration:none}
.hb-row:last-child{border-bottom:0}.hb-row-date{align-self:stretch;padding:28px 24px 28px 0;border-right:1px solid var(--hb-hair);color:var(--hb-muted);font-family:var(--mono);font-size:11px;font-variant-numeric:tabular-nums}.hb-row-copy{min-width:0;padding:26px 44px}.hb-row h2{margin:0 0 8px;color:var(--hb-ink);font-size:24px;font-weight:600;line-height:1.3;transition:color 120ms var(--hb-ease)}.hb-row-copy p{max-width:68ch;margin:0;color:var(--hb-body);font-size:14px;line-height:1.65;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}.hb-row-side{align-self:stretch;padding:26px 0 26px 24px;display:flex;flex-direction:column;justify-content:center;gap:8px;border-left:1px solid var(--hb-hair);color:var(--hb-muted);font-family:var(--mono);font-size:11px}.hb-row-side i{margin-top:4px;color:var(--hb-ink);font-size:18px;font-style:normal;transition:color 120ms var(--hb-ease),transform 120ms var(--hb-ease)}.hb-row:hover h2,.hb-row:hover .hb-row-side i{color:var(--accent)}.hb-row:hover .hb-row-side i{transform:translateX(4px)}
...
.blog-back{min-height:44px;margin:0 0 24px;display:inline-flex;align-items:center;color:var(--hb-muted);font-family:var(--mono);font-size:12px}.blog-back:hover{color:var(--accent)}
...
@media(max-width:900px){.hb-header-inner{padding-right:20px;padding-left:20px}.hb-nav{display:none}.hb-menu-button{display:inline-flex}.hb-featured{grid-template-columns:112px minmax(0,1fr)}.hb-featured-cover,.hb-terminal-cover{grid-column:2;min-height:260px;border-top:1px solid var(--hb-hair)}.hb-featured-copy{border-right:0}.hb-footer-inner{grid-template-columns:1fr}.hb-footer nav{justify-content:flex-start}.hb-copyright{grid-column:1}}
@media(max-width:640px){html{scroll-padding-top:64px}.hb-header,.hb-header-inner{height:64px}.hb-header-inner{padding-right:max(14px,env(safe-area-inset-right));padding-left:max(14px,env(safe-area-inset-left));gap:8px}.hb-brand{gap:6px}.hb-brand img{width:25px;height:25px}.hb-brand b{font-size:14px}.hb-actions{gap:6px}.hb-control,.hb-login,.hb-user{min-height:44px}.hb-language-trigger{min-width:68px;padding:0 9px}.hb-login{padding:0 12px;font-size:12px}.hb-user{width:44px;padding:3px}.hb-user-name{display:none}.hb-mobile-menu{top:63px}.hb-frame{padding-right:18px;padding-left:18px}.hb-intro .hb-frame{min-height:250px;padding-top:48px;padding-bottom:48px;display:block}.hb-micro{margin-bottom:28px}.hb-intro h1{font-size:36px;line-height:1.1}.hb-intro-copy>p{font-size:15px}.hb-ledger{padding-bottom:56px}.hb-featured{min-height:0;grid-template-columns:1fr}.hb-featured-meta{padding:22px 0 14px;flex-direction:row;border-right:0;border-bottom:1px solid var(--hb-hair)}.hb-featured-cover,.hb-terminal-cover{grid-column:1;grid-row:2;min-height:220px;border-top:0;border-bottom:1px solid var(--hb-hair)}.hb-featured-copy{grid-row:3;padding:30px 0 48px}.hb-featured h2{font-size:28px}.hb-read-arrow{right:0;bottom:16px}.hb-row{min-height:112px;padding:24px 0;grid-template-columns:1fr}.hb-row-date{align-self:auto;padding:0 0 10px;border:0}.hb-row-copy{padding:0}.hb-row h2{font-size:21px}.hb-row-side{padding:14px 0 0;flex-direction:row;align-items:center;gap:10px;border:0}.hb-row-side i{margin:0 0 0 auto}.blog-wrap{padding:40px 18px 72px}.article-title{font-size:34px;line-height:1.16}.article-byline{margin-bottom:28px}.article-cover{margin-bottom:36px}.article-content{font-size:17px}.article-content>p:first-child{font-size:18px}.article-content pre{margin-right:-8px;margin-left:-8px;padding:17px 16px}.article-content pre code{white-space:pre-wrap;overflow-wrap:anywhere}.blog-cta{margin-top:48px;padding:24px 20px}.hb-footer-inner{padding:36px 18px}.hb-footer nav{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0}.hb-footer nav a{border-bottom:1px solid var(--hb-hair)}}
@media(max-width:370px){.hb-brand b{display:none}.hb-language-trigger{min-width:62px}.hb-login{padding:0 10px}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.hb-language-trigger i,.hb-menu-button>span,.hb-menu-button>span::before,.hb-menu-button>span::after,.hb-featured h2,.hb-read-arrow,.hb-row h2,.hb-row-side i{transition:none!important}.hb-featured:hover .hb-read-arrow,.hb-row:hover .hb-row-side i{transform:none}}
`
